...
mod tests {
    ...
    /// nextgsim-ngap transfer decoder must read it. Leading 0x00 = the outer
    /// extensible SEQUENCE's APER extension-presence bit (ngap-04); the
    /// remaining 32 octets are the `ProtocolIE-Container`.
    const SMFD_SETUP_REQUEST_TRANSFER: [u8; 33] = [
