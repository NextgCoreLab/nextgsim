...
pub fn decode_ntn_timing_info(bytes: &[u8]) -> Result<NtnTimingInfo, NtnSupportError> {
    ...
    let satellite_type = match bytes[0] {
        ...
        _ => {
            return Err(NtnSupportError::CodecError(
                "Unknown satellite type".to_string(),
            ))
        }
    };
    ...
    let propagation_delay_us = u64::from_be_bytes(bytes[5..13].try_into().map_err(|_| {
        NtnSupportError::CodecError("Invalid propagation_delay_us bytes".to_string())
    })?);
    ...
    let validity_duration_ms = u32::from_be_bytes(bytes[21..25].try_into().map_err(|_| {
        NtnSupportError::CodecError("Invalid validity_duration_ms bytes".to_string())
    })?);
    let reference_time_ms =
        u64::from_be_bytes(bytes[25..33].try_into().map_err(|_| {
            NtnSupportError::CodecError("Invalid reference_time_ms bytes".to_string())
        })?);
