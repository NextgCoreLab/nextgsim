...
async fn test_gtp_encapsulation() {
    ...
    let (gtp_header, dest_addr) = tunnel_mgr
        .encapsulate_uplink(ue_id, psi, user_data.clone())
...
async fn test_gtp_echo() {
    ...
    // Create echo request
    let seq_num = 1234u16;
    let echo_req = GtpHeader::echo_request(0).with_sequence_number(seq_num);
    ...
    let echo_resp = GtpHeader::echo_response(0).with_sequence_number(seq_num);
