...
async fn test_multiple_ue_registration() {
    ...
    for i in 0..num_ues {
        let amf_ue_id = amf
            .simulate_initial_ue_message(i as u32, vec![])
            .await
            .unwrap();
...
async fn test_concurrent_ue_operations() {
    ...
    for i in 0..num_ues {
        ...
        let handle = tokio::spawn(async move {
            let amf_ue_id = amf_clone
                .simulate_initial_ue_message(i as u32, vec![])
                .await
                .unwrap();
            let imsi = format!("imsi-00101000000000{:02}", i + 1);
            amf_clone
                .simulate_registration_complete(amf_ue_id, &imsi)
                .await
                .unwrap();
            ...
            amf_clone
                .simulate_pdu_session_establish(amf_ue_id, 1, &ue_ip)
                .await
                .unwrap();
...
async fn test_multiple_gnbs() {
    ...
    amf.simulate_registration_complete(amf_ue_id_1, "imsi-001010000000001")
        .await
        .unwrap();
    amf.simulate_registration_complete(amf_ue_id_2, "imsi-001010000000002")
        .await
        .unwrap();
    amf.simulate_registration_complete(amf_ue_id_3, "imsi-001010000000003")
        .await
        .unwrap();
...
async fn test_independent_ue_state_machines() {
    ...
    amf.simulate_pdu_session_establish(ue_ids[0], 1, "10.45.0.2")
        .await
        .unwrap();
    amf.simulate_pdu_session_establish(ue_ids[0], 2, "10.45.0.3")
        .await
        .unwrap();

    // UE 1: Has 1 PDU session
    amf.simulate_pdu_session_establish(ue_ids[1], 1, "10.45.0.4")
        .await
        .unwrap();
    ...
    amf.simulate_pdu_session_establish(ue_ids[3], 1, "10.45.0.5")
        .await
        .unwrap();
    amf.simulate_pdu_session_release(ue_ids[3], 1)
        .await
        .unwrap();
    ...
    assert_eq!(
        ue3.pdu_sessions.len(),
        0,
        "UE 3 should have 0 PDU sessions after release"
    );
    ...
    assert!(
        amf.get_ue_context(ue_ids[1]).await.is_none(),
        "UE 1 should be released"
    );

    let ue0_after = amf
        .get_ue_context(ue_ids[0])
        .await
        .expect("UE 0 should still exist");
    assert_eq!(
        ue0_after.pdu_sessions.len(),
        2,
        "UE 0 should still have 2 PDU sessions"
    );

    let ue2_after = amf
        .get_ue_context(ue_ids[2])
        .await
        .expect("UE 2 should still exist");
    assert!(ue2_after.registered, "UE 2 should still be registered");

    let ue3_after = amf
        .get_ue_context(ue_ids[3])
        .await
        .expect("UE 3 should still exist");
