...
async fn test_ue_registration_basic() {
    ...
    amf.simulate_gnb_connect(1)
        .await
        .expect("Failed to connect gNB");
    amf.simulate_ng_setup_request(1)
        .await
    ...
    let amf_ue_id = amf
        .simulate_initial_ue_message(1, nas_pdu)
        .await
...
async fn test_ue_deregistration() {
    ...
    // Deregister UE
    amf.simulate_ue_context_release(amf_ue_id)
        .await
        .expect("Failed to release UE context");
...
async fn test_gnb_disconnect_during_registration() {
    ...
    // Disconnect gNB
    amf.simulate_gnb_disconnect(1)
        .await
        .expect("Failed to disconnect gNB");
