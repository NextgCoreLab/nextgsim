...
| NGS-5G-03 | UE RRC | Done: measurement events A1-A6 and inter-RAT B1/B2 with both the entering and the leaving inequality and a per-cell `cellsTriggeredList`, the conditional-handover runtime, and a secondary cell configured over the air (`sCellToAddModList`) so A6 has the reference it is defined against. Inter-RAT B1/B2 still have no E-UTRA measurement source | TS 38.331 | Complete | [#20](https://github.com/NextgCoreLab/nextgsim/issues/20), [#111](https://github.com/NextgCoreLab/nextgsim/issues/111), [#112](https://github.com/NextgCoreLab/nextgsim/issues/112) |
| NGS-5G-04 | gNB RRC | Done: the gNB broadcasts UPER MIB/SIB1 on BCCH (`si_broadcast_period_ms`) and the UE decodes them, so a cell's PLMN/TAC/NCI come off the air rather than being assumed. The residual hand-rolled RRC PDUs (re-establishment, resume, handover command, measurement report) are split out | TS 38.331 | Complete | [#21](https://github.com/NextgCoreLab/nextgsim/issues/21) |
