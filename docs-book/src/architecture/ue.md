...
4. **Spawn the actors.** `spawn_tasks` launches, in order: the **TUN** task
   ...
   **Ranging / MINT** tasks (and **Sidelink** only under the off-by-default
   `sidelink` Cargo feature); any enabled **6G** tasks; and last the
...
| `sidelink/` | (in-crate) | Rel-16/17 PC5 surface behind the off-by-default `sidelink` feature — real PC5-S Direct Communication, Model A/B discovery, UE-to-UE relay, **L2 UE-to-Network relay with the TS 38.351 SRAP adaptation layer** and the RRC `SidelinkUEInformation`; absent from a default build (see [6G & AI prototypes](#6g--ai-prototypes-pointers)) |
...
| `sidelink/` | **The real PC5 procedures run; two limits remain.** `EstablishPc5Link` sends a PC5-S `Direct Communication Request` and reaches `Active` only on the peer's `Direct Communication Accept` (TS 23.304 §6.4.3.1), with the peer's Layer-2 ID learned from the accept per step 4. Direct discovery is a real Model A announce / Model B solicit-response exchange with a ProSe Application Code filter (§6.3.1.2, §6.3.1.3), so a UE on another code is not discovered. UE-to-UE relay forwarding consults the live link table, so releasing a link stops the forwarding (§6.4.3.10). The UE emits an RRC `SidelinkUEInformation` (TS 38.331 §5.8.3, real UPER on `messageClassExtension.c2`) and the gNB answers with an `sl-ConfigDedicatedNR` in `RRCReconfiguration`, which the UE applies. `tests/src/sidelink_pc5_e2e.rs` drives three real tasks through discovery → unicast link → relay; the `sidelink-pc5` CI job runs it. **Limit 1:** there is no PC5 radio — the medium between UEs is an in-process channel, so the PDUs are real and the propagation is not. **L2 UE-to-Network relay also runs (issue #190):** a remote UE's end-to-end bearers are adapted through the TS 38.351 SRAP sublayer (`nextgsim_rlc::srap`) onto the relay's own Uu relay RLC channels, with the per-remote-UE bearer mapping and the local Remote UE ID signalled by the gNB in `sl-L2RelayUE-Config-r17` / `sl-L2RemoteUE-Config-r17` (TS 38.300 §16.12.2.1), and the relay granted a Mode-1 `sl-ScheduledConfig-r16` SL-RNTI plus an `sl-RLC-BearerToAddModList` channel it validates its mappings against. A UE asks for this by declaring `ue-Type-r17`. **Limit 2:** TS 38.351 itself is not in the vendored spec text, so the header's *fields and ranges* come from TS 38.300 §16.12.2.1 and the TS 38.331 ASN.1 (both vendored) while its *octet layout* is this simulator's own assignment — the same position #141 recorded for the PC5-S message-type octets. **Limit 3:** the relay's Uu leg is SRB1, not a numbered Uu relay RLC channel, because there is no MAC multiplexing here for one; the adaptation is real, the channel is the one that exists. **Limit 4:** no multi-hop relay, and no L3 relay (it needs an IP forwarding plane). Still absent from a default build (off-by-default `sidelink` feature), and additionally gated at run time by `UeConfig::prose_enabled`. Issues #54, #141, #190. |
