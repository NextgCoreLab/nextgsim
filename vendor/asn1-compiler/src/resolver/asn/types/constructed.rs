...
fn get_seq_component_for_object_set(
    ...
    for (key, object) in &objects.lookup_table {
        if let ResolvedObjectSetElement::Object(ref o) = object {
            ...
            // LOCAL FIX (nextgsim #185): accept ANY resolved type here, not only
            // `Asn1ResolvedType::Reference`.
            //
            // Upstream required the object-set field to resolve to a Reference,
            // i.e. to a *named* type. An information-object-class field written
            // inline as a base type -- in NGAP, `TYPE OCTET STRING (CONTAINING
            // <Transfer>)` -- resolves to `Asn1ResolvedType::Base` and was
            // therefore SILENTLY dropped from the generated OPEN-type enum: no
            // error, no warning, just a missing variant. Decoding a PDU that
            // carries such an IE then fails with `Key <id> Not Found`.
            //
            // In TS 38.413 V17.9 this hit 14 ProtocolIE entries, all of them MBS
            // (ids 300-306, 314-316, 348-350, 358), which is why nothing else in
            // this tree ever noticed. IE 304
            // (id-MulticastSessionActivationRequestTransfer) is one of the two
            // mandatory IEs of `MulticastSessionActivationRequest`, so the gNB
            // could not decode an MBS activation at all.
            //
            // Dropping the Reference test is purely additive to the generated
            // tree: the compiler already emits `OCTET STRING (CONTAINING X)` as
            // an unconstrained `OCTET-STRING` newtype when it appears as a
            // SEQUENCE field (e.g. PDUSessionResourceSetupItemSUReq), so the
            // resulting wire shape matches what a named alias would have given.
            // Verified against TS 38.413 V17.9: 19 IE variants gained, zero
            // removed, zero retyped, and the 4223-test workspace unchanged.
            if let Some(ResolvedFieldSpec::Type { ty: Some(ty) }) = field_ob {
                types.insert((key.1.clone(), key.0.clone()), (key.0.clone(), ty.clone()));
            }
        }
    }
    Ok(types)
