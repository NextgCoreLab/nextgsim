...
///
/// This is NOT the same thing as [`encode_normally_small_length_determinent_common`]
/// next door, and the difference is exactly one off-by-one that would corrupt
/// every extended CHOICE index if the two were confused. §11.6.1 encodes the
/// value `n` itself in six bits when `n <= 63`; a *length* determinant encodes
/// `n - 1`, because a length of zero is not expressible and so the range is
/// shifted. The length helper therefore also underflows on `value == 0`, which is
/// a legal extension index.
///
/// The bit layout, and why it is this and not something else: a leading `0` bit
/// says "small form", followed by the value in **six** bits (§11.6.1, range
/// 0..=63). A leading `1` bit says the value is encoded as a semi-constrained
/// whole number with a lower bound of 0 (§11.6.2).
///
/// Pairs with `decode_normally_small_non_negative_whole_number_common`, which
/// reads a bool then either six bits or a semi-constrained number with `lb = 0`.
/// That decoder is the specification this follows: the encoder must produce
/// exactly what it consumes, so the two are byte-for-byte inverses by
/// construction rather than by inspection.
pub(super) fn encode_normally_small_non_negative_whole_number_common(
    data: &mut PerCodecData,
    value: i128,
    aligned: bool,
) -> Result<(), PerCodecError> {
    if value < 0 {
        return Err(PerCodecError::new(
            PerCodecErrorCause::Generic,
            format!("A normally small non-negative whole number cannot be {value}"),
        ));
    }
    ...
    if value <= 63 {
        ...
        encode_semi_constrained_whole_number_common(data, 0, value, aligned)?;
    }
