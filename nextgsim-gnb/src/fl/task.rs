...
use crate::tasks::{FlAggregatorMessage, GnbTaskBase, Task, TaskMessage};
...
impl Task for FlAggregatorTask {
    ...
    async fn run(&mut self, mut rx: mpsc::Receiver<TaskMessage<Self::Message>>) {
        ...
        loop {
            match rx.recv().await {
                Some(TaskMessage::Message(msg)) => match msg {
                    FlAggregatorMessage::RegisterParticipant {
                        participant_id,
                        compute_capability,
                    } => {
                        debug!(
                            "FL: Registering participant {} (compute={})",
                            participant_id, compute_capability
                        );
                        self.aggregator
                            .register_participant(&participant_id, compute_capability);
                        self.participants.insert(participant_id, compute_capability);
                    }
                    FlAggregatorMessage::SubmitUpdate {
                        participant_id,
                        round,
                        gradients,
                        num_samples,
                        ...
                        debug!(
                            "FL: Update from {} for round {} ({} samples)",
                            participant_id, round, num_samples
                        );
                    ...
                    FlAggregatorMessage::DistributeModel { version, weights } => {
                        debug!(
                            "FL: Distributing model v{} ({} params)",
                            version,
                            weights.len()
                        );
        ...
        info!(
            "FL Aggregator task stopped, {} participants, round {}",
